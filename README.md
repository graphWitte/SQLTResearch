...
смотрим до оптимизации. Именно так база ищет данные без индексов
![до оптимизации](jpges/GenDoOptim.png)
**В итоге мы видим:**
- **Проблема** в том, что в плане выполнения используется **`Seq Scan`**, а это последовательное сканирование, в и тоге база данных считает **каждую** из **50 000** строк таблицы, а это очень медленно и неоптимизированно
- **Время** - Запрос занял **8.564ms**
Для решения нужно создать индекс по столбу `car_id`, чтобы база не перебирала всю таблицу вручную
![оптимизация](jpges/Optim.png)
Добавляю индекс по столбцу `car_id` чтобы ускорить поиск
![ВАУ!](jpges/wow.png)
...
**Стало быстрее** благодаря методу индексного сканирования **`Index Scan`**
